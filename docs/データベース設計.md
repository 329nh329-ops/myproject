...
erDiagram
    ...
    CARDS {
        bigint id PK
        bigint list_id FK "所属リスト（LISTS.id）"
        string title "タイトル（必須）"
        text description "説明文（任意）"
        string priority "優先度（高・中・低、enum）"
        date due_date "期限（任意）"
...
| list_id | BIGINT | FK（LISTS.id）, NOT NULL | 所属リスト |
| title | VARCHAR(50) | NOT NULL | タイトル |
| description | TEXT | NULL可 | 説明文 |
| priority | VARCHAR(10) | NOT NULL | 優先度。アプリケーション側ではenum（Priority: 高/中/低）として扱い、DBには`@Enumerated(EnumType.STRING)`により文字列（'高' / '中' / '低'）で保存する |
